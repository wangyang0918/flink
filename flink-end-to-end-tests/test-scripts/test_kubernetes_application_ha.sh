...
S3_ENDPOINT=$(echo $S3_ENDPOINT | sed s"/localhost/$(get_host_machine_address)/")
s3_setup hadoop
...
"$FLINK_DIR"/bin/flink run-application -d -t kubernetes-application \
    ...
    -Dhigh-availability.storageDir=s3://$IT_CASE_S3_BUCKET/flink-ha/$CLUSTER_ID \
    -Drestart-strategy=fixed-delay \
    -Drestart-strategy.fixed-delay.attempts=10 \
    -Dcontainerized.master.env.ENABLE_BUILT_IN_PLUGINS=flink-s3-fs-hadoop-$FLINK_VERSION.jar \
    -Dcontainerized.taskmanager.env.ENABLE_BUILT_IN_PLUGINS=flink-s3-fs-hadoop-$FLINK_VERSION.jar \
...
kubectl wait --for=delete configmap --all=true --timeout=60s
