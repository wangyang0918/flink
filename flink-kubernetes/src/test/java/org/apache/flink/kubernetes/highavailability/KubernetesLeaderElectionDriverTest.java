...
public class KubernetesLeaderElectionDriverTest extends KubernetesHighAvailabilityTestBase {
	...
	@Test
	public void testLeaderConfigMapModifiedExternally() throws Exception {
		new Context() {{
			runTest(
				() -> {
					leaderCallbackGrantLeadership();
					...
					// The leader change should be notified to leaderElectionEventHandler
		...
		new Context() {{
			runTest(
				() -> {
					...
					final FlinkKubeClient.WatchCallbackHandler<KubernetesConfigMap> callbackHandler =
						getLeaderElectionConfigMapCallback();
					callbackHandler.onDeleted(Collections.singletonList(getLeaderConfigMap()));
