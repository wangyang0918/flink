...
public class KubernetesWorkerNode implements ResourceIDRetrievable {
	...
	public long getAttempt() throws Exception {
		Matcher m = TASK_MANAGER_POD_PATTERN.matcher(resourceID.toString());
		boolean mat = m.find();
		if (mat) {
			return Long.parseLong(m.group(1));
		} else {
			throw new Exception("Error to parse KubernetesWorkerNode from " + resourceID.toString());
		}
	}
}
